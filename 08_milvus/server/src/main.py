...
@app.get('/search_en')
async def do_en_search_api(table_name: str = None, query_sentence: str = None):
    try:
        _, title, text, _ = search_in_milvus(table_name,query_sentence,MODEL, MILVUS_CLI, MYSQL_CLI, True)
        res = []
        for p, d in zip(title, text):
            dicts = {'title': p, 'content':d}
            res+=[dicts]
